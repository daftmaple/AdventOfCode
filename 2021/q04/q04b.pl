...
sub findResult {
    ...
    my @finishedBingo = (0) x @$bingoTablesRef;

    # print Dumper @finishedBingo;
    # exit;
    ...
    while ($iteration < scalar(@bingoNumberSequence)) {
        ...
        foreach my $bingoTableIndex (0..(scalar(@$bingoTablesRef) - 1)) {
            my $bingoTable = $bingoTablesRef->[$bingoTableIndex];

            if ($finishedBingo[$bingoTableIndex] == 0 and isBingo($bingoTable, $iteration)) {
                $finishedBingo[$bingoTableIndex] = 1;
            }
            
            if (sumArray(\@finishedBingo) == scalar(@$bingoTablesRef)) {
